...
import type { PostgreSqlConnectionProps } from './postgresql-connection.interface';
...
/**
 * A SageMaker Unified Studio PostgreSQL database connection.
 *
 * Creates a Glue connection of type POSTGRESQL with Spark and Athena compute,
 * targeting a PostgreSQL database (Amazon RDS, Aurora PostgreSQL, or on-premises).
 *
 * @see https://docs.aws.amazon.com/sagemaker-unified-studio/latest/userguide/data-connections-iam-based-domains.html
 */
export class PostgreSqlConnection extends GlueConnection {
  constructor(scope: Construct, id: string, props: PostgreSqlConnectionProps) {
    const port = props.port ?? DEFAULT_PORT;

    if (port < 1 || port > 65535) {
      throw new Error(`PostgreSqlConnection port must be between 1 and 65535, got ${port}.`);
    }

    if (props.securityGroupIds.length === 0) {
      throw new Error('PostgreSqlConnection securityGroupIds must contain at least one security group.');
    }

    if (props.kmsKeyArn && !props.secretArn) {
      throw new Error('PostgreSqlConnection kmsKeyArn is only valid when secretArn is provided.');
    }
    ...
    super(scope, id, {
      ...
      connectionType: GlueConnectionType.POSTGRESQL,
