...
import type { SqlServerConnectionProps } from './sqlserver-connection.interface';
...
/**
 * A SageMaker Unified Studio SQL Server database connection.
 *
 * Creates a Glue connection of type SQLSERVER with Spark and Athena compute,
 * targeting a Microsoft SQL Server database (Amazon RDS or on-premises).
 *
 * @see https://docs.aws.amazon.com/sagemaker-unified-studio/latest/userguide/data-connections-iam-based-domains.html
 */
export class SqlServerConnection extends GlueConnection {
  constructor(scope: Construct, id: string, props: SqlServerConnectionProps) {
    const port = props.port ?? DEFAULT_PORT;

    if (port < 1 || port > 65535) {
      throw new Error(`SqlServerConnection port must be between 1 and 65535, got ${port}.`);
    }

    if (props.securityGroupIds.length === 0) {
      throw new Error('SqlServerConnection securityGroupIds must contain at least one security group.');
    }

    if (props.kmsKeyArn && !props.secretArn) {
      throw new Error('SqlServerConnection kmsKeyArn is only valid when secretArn is provided.');
    }
    ...
    super(scope, id, {
      ...
      connectionType: GlueConnectionType.SQLSERVER,
